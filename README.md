...
- 从上游 YAML frontmatter 读取 `name` 和 `description`；
- 生成 ChatGPT Skill 要求的 frontmatter，只保留这两个字段；
...
`validate_conversion.py` 会重新对照上游快照，确认 `name`、`description`、核心正文和详细规则 payload 没有丢失或被重写。
...
创建一个空仓库（建议名 `humanizer-zh-chatgpt`），然后把本项目推送到 `main`。第一次 push 后，Actions 会运行 CI；之后 `Sync upstream Humanizer-zh` 会按计划自动工作。
